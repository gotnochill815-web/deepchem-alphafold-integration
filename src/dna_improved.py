...
def extract_sequence_from_pdb(fname):
...
def featurize(fname):
    ...
    seq = extract_sequence_from_pdb(fname)
    L = max(len(seq),1)

    fracK = seq.count("K")/L
    fracR = seq.count("R")/L
    fracH = seq.count("H")/L
    fracBasic = (seq.count("K")+seq.count("R")+seq.count("H"))/L
    fracAcid = (seq.count("D")+seq.count("E"))/L
    netCharge = fracBasic - fracAcid
